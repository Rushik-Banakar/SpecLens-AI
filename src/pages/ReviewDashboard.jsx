...
export default function ReviewDashboard({ onBackToUpload, uploadedDocs = [], extractedRequirements = null, detectedCollisions = null }) {
  ...
  const documentsList = uploadedDocs && uploadedDocs.length > 0 
    ? uploadedDocs.map((doc, idx) => ({
        id: doc.id || `uploaded-doc-${idx}`,
        name: doc.name,
        size: doc.size,
        type: doc.type.toUpperCase()
      }))
    : mockProjectData.documents;

  // ── Derive live metrics from real AI output ─────────────────────────────
  const liveCollisions = detectedCollisions?.collisions || [];
  const hasRealCollisions = liveCollisions.length > 0 || detectedCollisions !== null;

  const liveCritical = detectedCollisions?.stats?.critical_count ?? null;
  const liveHigh     = detectedCollisions?.stats?.high_count     ?? null;
  const liveMedium   = detectedCollisions?.stats?.medium_count   ?? null;
  const liveHealthScore = detectedCollisions?.stats?.health_score ?? null;
  const liveCollisionCount = detectedCollisions?.collisions?.length ?? null;

  // Sidebar contradiction badge: use real count if available, else mock
  const contradictionBadgeCount = liveCollisionCount !== null
    ? liveCollisionCount
    : mockProjectData.findings.filter(f => f.category === 'Contradictions').length;

  // Dashboard metric cards
  const displayHealthScore   = liveHealthScore !== null ? liveHealthScore : mockProjectData.healthScore;
  const displayCriticalCount = liveCritical    !== null ? liveCritical    : mockProjectData.metrics.criticalIssues;
  const displayMediumCount   = liveMedium      !== null ? (liveHigh || 0) + liveMedium : mockProjectData.metrics.mediumIssues;
  const displayReqCount      = extractedRequirements?.stats?.total_requirements ?? mockProjectData.requirementsCount;
  ...
  const filteredFindings = mockProjectData.findings.filter(f => {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row relative">
      ...
      <aside className={`w-full md:w-64 border-r border-slate-900 bg-slate-900/30 flex flex-col justify-between fixed md:sticky top-0 h-[calc(100vh-57px)] md:h-screen z-20 transition-all duration-300 ${
          ...
          <nav className="space-y-1">
            ...
            >
              ...
              <span className="text-xs bg-amber-950/40 border border-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded-full font-bold">
                {mockProjectData.findings.filter(f => f.category === 'Ambiguities').length}
              </span>
            </button>
            ...
            >
              ...
              <span className="text-xs bg-emerald-950/40 border border-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded-full font-bold">
                {mockProjectData.findings.filter(f => f.category === 'Missing Requirements').length}
              </span>
            </button>
            ...
            >
              ...
              <span className="text-xs bg-sky-950/40 border border-sky-500/20 text-sky-400 px-1.5 py-0.5 rounded-full font-bold">
                {mockProjectData.findings.filter(f => f.category === 'Dependencies').length}
              </span>
            </button>
        ...
        <div className="p-6 border-t border-slate-900 space-y-3 bg-slate-950/40">
          <button 
            onClick={() => alert("Report compiled successfully! Mock PDF download started.")}
      ...
      <main className="flex-1 min-w-0 overflow-y-auto px-6 py-8 md:p-8 space-y-8 pb-24">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-900 pb-6">
          <div>
            ...
            <h1 className="text-xl md:text-2xl font-black mt-1">{mockProjectData.projectName}</h1>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="p-5 rounded-2xl border border-slate-900 bg-slate-900/10 flex items-center justify-between">
            <div>
              ...
              <p className={`text-[10px] mt-2 font-medium ${displayHealthScore >= 80 ? 'text-emerald-400' : displayHealthScore >= 60 ? 'text-amber-400' : 'text-rose-400'}`}>
                {displayHealthScore >= 80 ? 'Healthy' : displayHealthScore >= 60 ? 'Needs Attention' : 'Critical Risk'}
              </p>
            </div>
            ...
            <div className="relative w-16 h-16 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle 
                  cx="32" cy="32" r="26" 
                  fill="transparent" 
                  stroke={displayHealthScore >= 80 ? '#10b981' : displayHealthScore >= 60 ? '#f59e0b' : '#ef4444'}
                  strokeWidth="4" 
                  strokeDasharray="163.3"
                  strokeDashoffset={163.3 - (163.3 * displayHealthScore) / 100}
                />
              </svg>
              <span className="absolute text-xs font-bold text-indigo-300">{displayHealthScore}%</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            {activeTab === 'summary' && (
              <div className="p-6 rounded-2xl border border-slate-900 bg-slate-900/20 space-y-4">
                ...
                <p className="text-sm text-slate-300 leading-relaxed">
                  {mockProjectData.executiveSummary}
                </p>
                    ...
                    {documentsList.map(doc => (
                      <div key={doc.id} className="p-3 rounded-xl border border-slate-900 bg-slate-950/80 flex items-center justify-between text-xs">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="font-semibold text-slate-300 truncate max-w-[120px]" title={doc.name}>{doc.name}</span>
                        </div>
            ...
            {activeTab === 'contradictions' && (
              <CollisionsPanel
                ...
                isLoading={false}
                error={detectedCollisions?.error || null}
              />
            )}
                ...
                <div className="w-full overflow-x-auto">
                  <div className="min-w-[620px] max-w-3xl mx-auto border border-slate-900 rounded-xl bg-slate-950 p-4 relative shadow-inner">
                    <svg viewBox="0 0 800 550" className="w-full h-auto select-none">
                      ...
                      {/* Render Links */}
                      {mockProjectData.graphData.links.map((link, idx) => {
                        const sourceNode = mockProjectData.graphData.nodes.find(n => n.id === link.source);
                        const targetNode = mockProjectData.graphData.nodes.find(n => n.id === link.target);
                      ...
                      {/* Render Nodes */}
                      {mockProjectData.graphData.nodes.map(node => {
                        const isDocument = node.type === 'document';
                        const isHovered = hoveredNodeId === node.id;
                        const correspondingFinding = !isDocument 
                          ? mockProjectData.findings.find(f => f.title.toLowerCase().includes(node.label.toLowerCase()) || node.label.toLowerCase().includes(f.title.toLowerCase()) || (node.id === 'ISS-1' && f.id === 'SPEC-001') || (node.id === 'ISS-2' && f.id === 'SPEC-002') || (node.id === 'ISS-3' && f.id === 'SPEC-005') || (node.id === 'ISS-4' && f.id === 'SPEC-007'))
                          : null;
            ...
            {selectedFinding ? (
              <div className="p-6 rounded-2xl border border-slate-900 bg-slate-900/10 sticky top-8 space-y-6 text-left">
                ...
                <div className="space-y-1">
                  ...
                  <h3 className="text-lg font-extrabold text-slate-100">{selectedFinding.title}</h3>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 font-mono text-xs text-left">
                    <div className="p-3 bg-slate-950 rounded-xl border border-slate-900/60 relative group">
                      <span className="text-[9px] font-bold text-slate-500 uppercase block mb-1 truncate">{selectedFinding.documents[0]}</span>
